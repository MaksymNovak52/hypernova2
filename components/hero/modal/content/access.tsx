...
export const AccessStep: React.FC<AccessStepProps> = ({
  ...
  <div className="flex justify-between items-center w-[300px] px-4">
    ...
    <div>
      {isEditing ? (
        <div className="flex flex-row gap-1 items-center w-[240px] rounded-md  bg-[rgba(255,255,255,0.05)]  border border-[#BBBBBA] px-2 ">
          <input
            autoFocus
            value={value}
            ...
            className=" h-[26px] text-xs  w-[95%] bg-transparent text-[#BBBBBA] outline-none"
...
type StepItem = {
  ...
  label: string;
  status: "Connected" | "Connect";
  value: string; // те, що ввів користувач
};
...
const PriorityAccess: React.FC = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-[#FFFFFF0D]/[0.05] py-4 rounded-xl w-[300px] border border-[#494848] mx-auto">
      ...
      <div className="flex flex-col space-y-4">
        {steps.map((step, idx) => (
          <AccessStep
            ...
            value={step.value}
            onStartEdit={() => {
              // якщо інший редагується і там пусто — просто переключаємось
              setEditingIdx((prev) => (prev === idx ? prev : idx));
            }}
            onChangeValue={(v) => changeValue(idx, v)}
            onCommit={() => commit(idx)} // Enter
            onCancel={() => cancel(idx)} // Esc
          />
        ))}
      </div>
    </div>
